...
public class StronglyConnectedComponents {
	...
	public static Stack<Vertex> DFS(Graph g) {
		...
		while (it.hasNext()) {
			Vertex u = it.next();
			if (!u.seen&&u!=null)
				DFSVisit(u, topStack, true);//calling DFS visit on every unseen node. True represents DFS on G (First DFS)
		}
			
		return topStack;
	}

	public static void DFSVisit(Vertex u, Stack<Vertex> topStack, boolean transpose) {
		...
		while (neighbouringEdges.hasNext()) {
			...
			if (!v.seen) {
				v.parent = u;
				DFSVisit(v, topStack,transpose);//recursive call
				}
			}
	 ...
	 public static void main(String ar[]) throws FileNotFoundException {
			Graph g = Graph.readGraph(
					new Scanner(new File("D:\\Spring 2016\\Implementation of Advanced Data Structures\\sp2input.txt")),
					true);
			try
			{
